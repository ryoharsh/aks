...
import type { MirrorDataRepository } from "./mirror.repository.ts";
...
export async function processMirrorTurn(input: {
    ...
    const runAI = async <T>(request: AIRequest, validate: (content: string) => T) => {
    ...
    const completeSuccessfulRun = (generated: { runId: string; result: AIResult }) => input.repository.updateAIRun(generated.runId, {
        status: "succeeded",
    ...
    const processMemory = async (signals: Array<{ signalType: string }>) => {
        ...
        try {
            const result = await evaluateMemory({ repository: input.memoryRepository, ai, conversationId: input.conversationId, userMessageId: input.userMessageId, observedAt: userMessage.createdAt, currentSignals: signals });
    ...
    const processPatterns = async (signals: Array<{ signalType: string }>) => {
        ...
        try {
            return await analyzePatterns({ repository: input.patternRepository, ai, conversationId: input.conversationId, userMessageId: input.userMessageId, observedAt: userMessage.createdAt, currentSignals: signals });
        } catch {
            return [];
        }
    };
    ...
    if (existingReply) {
        ...
        const signals = existingReply.metadata.task_version === "safety_response_v1" ? [] : await extractSignals();
    ...
    if (requiresCrisisResponse(userMessage.content)) {
        ...
        return { conversationId: input.conversationId, response: { response: { text: crisisResponse }, followUp: null }, assistantMessage, signalsSaved: 0, signals: [], memoryCandidates: [], patternActions: [] };
    }
    ...
    const response = validated.safety.risk === "imminent"
    ...
    const [memoryCandidates, patternActions] = await Promise.all([processMemory(signals), processPatterns(signals)]);
    return { conversationId: input.conversationId, response, assistantMessage, signalsSaved: signals.length, signals, memoryCandidates, patternActions };
}
