...
import { useDataAccess, type DataAccessSource } from "@/hooks/useDataAccess";
...
const ACCESS_ITEMS = copy.dataAccess.items;

/** Truthful status line — only what the registry actually reports. */
function sourceStatusCopy(source: DataAccessSource): string {
    if (!source.connected) return copy.dataAccess.connectedNeedsPermission;
    const synced = formatRelativeTime(source.lastSyncedAt);
    return synced ? copy.dataAccess.connectedSynced(synced) : copy.dataAccess.connectedNotSynced;
}
    ...
    return (
        <View className="flex-1 bg-background">
            <Animated.View entering={FadeInDown.duration(400)} className="h-16 flex-row items-center px-5">
                ...
                <AppText variant="title" className="text-text-high">{copy.dataAccess.header}</AppText>
            </Animated.View>
            <ScrollView showsVerticalScrollIndicator={false} contentContainerClassName="px-5 pb-24">
                <Animated.View entering={FadeInDown.duration(500).delay(70)} className="mt-5">
                    <AppText variant="display" className="text-text-high">{copy.dataAccess.title}</AppText>
                    <AppText className="mt-3 leading-6 text-text-low">{copy.dataAccess.description}</AppText>
                </Animated.View>
                ...
                <Animated.View entering={FadeInUp.duration(500).delay(130)} className="mt-8">
                    <View className="mb-3 flex-row items-center">
                        ...
                        <AppText variant="caption" className="ml-2 flex-1 tracking-[1.5px] text-text-low">{copy.dataAccess.sourcesSection}</AppText>
                    </View>
                    {loading && (
                        <View className="rounded-[28px] border border-border bg-surface p-5">
                            <AppText className="text-text-low">{copy.dataAccess.loadingSources}</AppText>
                        </View>
                    )}
                    {!loading && error && (
                        <View className="rounded-[28px] border border-border bg-surface p-5">
                            <AppText className="text-text-low">{copy.dataAccess.loadError}</AppText>
                            <AppText className="mt-1 text-text-medium">{error}</AppText>
                        </View>
                    )}
                    {!loading && !error && sources.length === 0 && (
                        <View className="rounded-[28px] border border-border bg-surface p-5">
                            <AppText variant="button" className="text-text-high">{copy.dataAccess.emptyTitle}</AppText>
                            <AppText className="mt-2 leading-6 text-text-low">{copy.dataAccess.emptyBody}</AppText>
                        </View>
                    )}
                    {!loading && !error && sources.length > 0 && (
                        <View className="gap-2">
                            {sources.map((source) => (
                                <View key={source.sourceType} className="flex-row items-center rounded-[28px] border border-border bg-surface p-5">
                                    ...
                                    <View className="ml-3 flex-1">
                                        <AppText variant="button" className="text-text-high">{source.name}</AppText>
                                        <View className="mt-1 flex-row items-center">
                                            <HugeiconsIcon icon={Database01Icon} size={14} color={muted} />
                                            <AppText className="ml-1.5 text-text-medium">{sourceStatusCopy(source)}</AppText>
                                        </View>
                                    </View>
                                </View>
                            ))}
                ...
                <Animated.View entering={FadeInUp.duration(500).delay(190)} className="mt-8 gap-3">
                    <AppText variant="caption" className="tracking-[1.5px] text-text-low">{copy.dataAccess.holdsSection}</AppText>
                ...
                <Animated.View entering={FadeInUp.duration(500).delay(250)} className="mt-6 rounded-[28px] border border-border bg-surface p-5">
                    ...
                    <AppText variant="title" className="text-text-high">{copy.dataAccess.controlTitle}</AppText>
                    <AppText className="mt-3 leading-6 text-text-low">{copy.dataAccess.controlBody}</AppText>
                </Animated.View>
            </ScrollView>
        </View>
    );
